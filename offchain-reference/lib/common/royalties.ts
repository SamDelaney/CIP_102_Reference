...
import { asChainAddress, ChainAddressSchema } from "./chain.js";
...
export const RoyaltyRecipientShape =
  RoyaltyRecipientSchema as unknown as RoyaltyRecipientType;
...
export const RoyaltyFlagSchema = Data.Object({
  royalty_included: Data.Integer(),
});
...
export function addCip102RoyaltyToTransaction(
  ...
  const royaltyOutputDatum: OutputDatum = {
    kind: "inline",
    value: royaltyDatum as CBORHex,
  };

  tx.mintAssets(royaltyAsset, redeemer).pay.ToAddressWithData(
    address,
    royaltyOutputDatum,
    royaltyAsset
  );
